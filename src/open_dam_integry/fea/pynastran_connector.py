...
def run_pynastran_or_fallback(height_m: float, width_m: float, surcharge_kPa: float) -> Dict[str, float]:
    if PYNASTRAN_AVAILABLE:
        # A real PyNastran model would be executed here; returning fallback for generality.
        return pseudo_fea_slope_response(height_m, width_m, surcharge_kPa) | {"backend": "pynastran"}
